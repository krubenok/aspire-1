    ...
    ILogger<AspireSkillsInstaller> logger) : IAspireSkillsInstaller
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var asset = FindGitHubReleaseAsset(release, assetKind, version);
            if (asset is null)
            {
    ...
    private GitHubReleaseAsset? FindGitHubReleaseAsset(GitHubReleaseInfo release, AgentAssetKind assetKind, string version)
    {
    ...
    private IEnumerable<string> GetGitHubReleaseAssetNameCandidates(AgentAssetKind assetKind, string version)
    {
        var unprefixedVersion = version.StartsWith('v') || version.StartsWith('V') ? version[1..] : version;
        var prefixedVersion = $"v{unprefixedVersion}";
        var archiveAssetNamePrefix = bundleProvider.GetArchiveAssetNamePrefix(assetKind);

        foreach (var archiveExtension in new[] { ".zip", ".tar.gz", ".tgz" })
        {
            yield return $"{archiveAssetNamePrefix}-{prefixedVersion}{archiveExtension}";
            yield return $"{archiveAssetNamePrefix}-{unprefixedVersion}{archiveExtension}";
        }
    }
